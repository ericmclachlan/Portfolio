    ...
    {   
            ...
            , ref string valueAsText)
        {
            if (property.PropertyType.IsAssignableFrom(typeof(string)))
            {
                ...
                {
                    var fileUri = new Uri(valueAsText);
            ...
            else if (property.PropertyType.IsAssignableFrom(typeof(int)))
            {
                ...
                {
                    string errorMessage = string.Format($"Parameter '{property.Name}' is not a valid integer value.");
                    throw new Exception(errorMessage);
                }
                if (commandParameterType == CommandParameterType.NonNegativeInteger && value < 0)
                {
                    string errorMessage = string.Format($"Parameter '{property.Name}' must be a non-negative integer.");
                    throw new Exception(errorMessage);
                }
